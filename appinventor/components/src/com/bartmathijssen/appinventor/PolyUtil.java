...
public final class PolyUtil {
  ...
  private static Path backingPath = new Path();
  private static RectF tempCornerArcBounds = new RectF();

  /**
   * Draws a regular polygon.
   *
   * <p>Note that this method is not thread safe. This is not an issue if (as is typical) all
  ...
  public static void drawPolygon(
    ...
    constructPolygonPath(
        backingPath, sideCount, centerX, centerY, outerRadius, cornerRadius, rotation);
   ...
   * @param rotation the rotation of the polygon in degrees
   */
  public static void constructPolygonPath(
    ...
    if (inRadius < cornerRadius) {
      ...
      if (abs(cornerRadius) < 0.01) {
        constructNonRoundedPolygonPath(path, sideCount, centerX, centerY, outerRadius);
      } else {
        constructRoundedPolygonPath(path, sideCount, centerX, centerY, outerRadius, cornerRadius);
      }
  ...
  private static void constructNonRoundedPolygonPath(
      @NonNull final Path path,
      @IntRange(from = 3) final int sideCount,
      final float centerX,
      final float centerY,
  ...
  private static void constructRoundedPolygonPath(
      @NonNull final Path path,
      @IntRange(from = 3) final int sideCount,
      final float centerX,
      final float centerY,
    ...
    final double distanceToCornerArcCenter =
        outerRadius - cornerRadius / sin(toRadians(halfInteriorCornerAngle));

    for (int cornerNumber = 0; cornerNumber < sideCount; cornerNumber++) {
      final double angleToCorner = cornerNumber * (360.0 / sideCount);
      final float cornerCenterX =
          (float) (centerX + distanceToCornerArcCenter * cos(toRadians(angleToCorner)));
      final float cornerCenterY =
          (float) (centerY + distanceToCornerArcCenter * sin(toRadians(angleToCorner)));
