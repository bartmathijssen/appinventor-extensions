package com.bartmathijssen.appinventor;
...
@DesignerComponent(
    version = 1,
    description =
        "This Extension was created with the AppyBuilder Code Editor.<br>"
...
public class DrawingCanvas extends AndroidNonvisibleComponent {
  ...
  public DrawingCanvas(ComponentContainer container) {
    ...
    view = new SimpleDrawingView(activity);
    linePath = new Path();
    setupPaint();
  ...
  public void DrawStar(int centerX, int centerY, int radius, boolean fill) {
    ...
    for (int i = 11; i != 0; i--) {
      ...
      if (i == 11) {
        path.moveTo(
            (float) ((r * Math.sin(omega)) + correctedX),
            (float) ((r * Math.cos(omega)) + correctedY));
      }
  ...
  /**
   * Draws a shape on the canvas. pointList should be a list contains sub-lists with two number
   * which represents a coordinate. The first point and last point does not need to be the same.
   * e.g. ((x1 y1) (x2 y2) (x3 y3)) When fill is true, the shape will be filled.
   *
   * @param pointList A list of points, should contains sub-lists with two number which represents a
   *     coordinate. The first point and last point does not need to be the same. e.g. ((x1 y1) (x2
   *     y2) (x3 y3))
  ...
  public void FloodFill(int x, int y, int tolerance) { // , Object replacementColor
    ...
    if (texture) {
      ...
      Drawable bottomLayer = new BitmapDrawable(activity.getResources(), b2);
      Drawable topLayer = new BitmapDrawable(activity.getResources(), b);
  ...
  public void ColoringBookFill(
      int x,
      int y,
      Object ignoreColor,
      int ignoreTolerance,
      int fillTolerance) { // , Object replacementColor
    float correctedX = x * Density();
    float correctedY = y * Density();
    final Point p1 = new Point();
    p1.x = (int) correctedX; // x co-ordinate where the user touches on the screen
  ...
  public void DrawScaledToFitImage(String imageFile) {
    ...
    Bitmap scaledBitmap =
        Bitmap.createScaledBitmap(bitmap, originalBitmapWidth, originalBitmapHeight, false);
  ...
  public void CropAndSave(int x1, int y1, int x2, int y2, String filePath) {
    ...
    int height = Math.round(correctedY2) - Math.round(correctedY1);
    Bitmap bitmap =
        Bitmap.createBitmap(b, Math.round(correctedX1), Math.round(correctedY1), width, height);
  ...
  public void FontTypefaceCustom(String fontFileName) {
    ...
    if (!fontFileName.contains("//")) {
      localTypeface1 = Typeface.createFromAsset(activity.getAssets(), fontFileName);
    } else if (fontFileName.startsWith("file:///android_asset/")) {
      String[] fontFile = fontFileName.split("file:///android_asset/");
      localTypeface1 = Typeface.createFromAsset(activity.getAssets(), fontFile[1]);
    } else {
  ...
  public void DrawVerticalGradientCircle(
      int centerX, int centerY, float radius, boolean fill, YailList colorList) {
    ...
    p.setShader(
        new LinearGradient(
            correctedX - correctedR,
            correctedY - correctedR,
            correctedX - correctedR,
            correctedY + correctedR,
            colors,
            null,
  ...
  public void DrawHorizontalGradientCircle(
      int centerX, int centerY, float radius, boolean fill, YailList colorList) {
    String[] strArray = colorList.toStringArray();
    colors = new int[strArray.length];
    for (int i = 0; i < strArray.length; i++) {
  ...
  public void DrawPolygon(
    ...
    // clear the prior shape. Is this needed? If not mentioned, then in blocks, we'll first need to
    // do Canvas.Clear, then call this method
  ...
  public void DrawOval(int x, int y, int height, int width, boolean fill) {
    ...
    RectF rect =
        new RectF(
            correctedX - correctedWidth,
            correctedY - correctedHeight,
            correctedX + correctedWidth,
            correctedY + correctedHeight);
  ...
  public void DrawRectangle(int x, int y, int height, int width, boolean fill) {
    ...
    RectF rect =
        new RectF(
            correctedX, correctedY, correctedX + correctedWidth, correctedY + correctedHeight);
  ...
  public void DrawingUp(int x, int y) {
    ...
    if (!zoom) {
      ...
      Paint p = new Paint(paint);
      p.setStyle(Paint.Style.STROKE);
      // future implementation for blurred edges on line
      c.drawPath(linePath, p);
    }
    linePath.reset();
    this.draw = false;
  ...
  public class SimpleDrawingView extends View {
    ...
    protected void onDraw(Canvas canvas) {
      ...
      if (rectangle && !zoom) {
        ...
        po.setStyle(cfill ? Paint.Style.FILL : Paint.Style.STROKE);
        RectF rect =
            new RectF(Math.min(cx, nx), Math.min(cy, ny), Math.max(nx, cx), Math.max(ny, cy));
  ...
  public class QueueLinearFloodFiller {
    ...
    public void floodFill(int x, int y) {
      ...
      while (ranges.size() > 0) {
        ...
        for (int i = range.startX; i <= range.endX; i++) {
          ...
          if (range.Y > 0 && (!pixelsChecked[upPxIdx]) && CheckPixel(upPxIdx)) LinearFill(i, upY);
    ...
    protected boolean CheckPixel(int px) {
      ...
      return (red >= (startColor[0] - tolerance[0])
          ...
          && blue <= (startColor[2] + tolerance[2]) // //
          && alpha >= (startColor[3] - tolerance[3])
          && alpha <= (startColor[3] + tolerance[3])); // //
    }
  ...
  private boolean allowFill(int ignoreColor, int targetColor, int allowTolerance) {
    int[] tolerance = new int[] {allowTolerance, allowTolerance, allowTolerance, allowTolerance};
    ...
    return !(red >= (iColor[0] - tolerance[0])
        && red <= (iColor[0] + tolerance[0]) // //
        && green >= (iColor[1] - tolerance[1])
        && green <= (iColor[1] + tolerance[1]) // //
        && blue >= (iColor[2] - tolerance[2])
        && blue <= (iColor[2] + tolerance[2]) // //
        && alpha >= (iColor[3] - tolerance[3])
        && alpha <= (iColor[3] + tolerance[3])); // //
  }
    ...
    if (texture) {
      if (mirror) {
        if (!TextureImagePath.startsWith("//")) {
          Bitmap bitmap = getBitmapFromAsset(activity, TextureImagePath);
          ...
          Bitmap bitmap = getBitmapFromAsset(activity, textureFile[1]);
          Shader shader = new BitmapShader(bitmap, Shader.TileMode.MIRROR, Shader.TileMode.MIRROR);
          paint.setShader(shader);
        } else {
